...
class lineDataset(Dataset):
    def __init__(self, tokenizer, args, logger, file_type='test', block_size=1):
        ...
        for data in datas:
            ...
            self.rec_id.append(data["rec_id"])
            self.recitation_code.append(data["recitation_code"])
            self.input_code_id.append(data["input_code_id"])
            self.input_code.append(data["input"])
            
    def __len__(self):
        return len(self.inputs)

    def __getitem__(self, item):
        return torch.tensor(self.inputs[item]), self.rec_id[item], self.recitation_code[item], self.input_code_id[item], self.input_code[item]
